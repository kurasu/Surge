#include <preset-discovery/preset-discovery.h>
...
void loadXMLForPresetDiscovery(preset_discovery_metadata_receiver_t receiver, const void *data, int datasize, bool preset)
{
    TiXmlDocument doc;
    if (datasize)
    {
        ...
        *(temp + datasize) = 0;
        ...
        if (patch)
        {
            ...
            if (meta)
            {
                ...
                if (s)
                    receiver->set_description(s);

                s = meta->Attribute("author");
                if (s)
                    receiver->add_creator(s);
            }
        }
        else
        {
...
void loadPatchForPresetDiscovery(preset_discovery_metadata_receiver_t receiver, const void *data, int datasize,
                      bool preset)
{
    if (datasize <= 4)
        return;
    ...
    if (!memcmp(ph->tag, "sub3", 4))
    {
        ...
        loadXMLForPresetDiscovery(receiver, dr, ph->xmlsize, preset);
    }
    else
    {
        loadXMLForPresetDiscovery(receiver, data, datasize, preset);
    }
}

void surgePresetMetadataParser(const char *path, preset_discovery_metadata_receiver_t receiver,
                               void *arg)
{
    ...
    {
        loadPatchForPresetDiscovery(receiver, data, cs, true);
    }
    free(data);
}

void preset_discovery_init(const preset_discovery_host *host, int host_version)
{
    preset_discovery_plugin_id_set_t pluginIdSet =
        host->create_plugin_id_set("Vember Audio", "Surge");

    host->plugin_id_set_add_vst3_id(pluginIdSet, SurgeControllerUID.toTUID());
    host->plugin_id_set_add_vst2_id(pluginIdSet, 'cjs3');
    host->plugin_id_set_set_default_device_type(pluginIdSet, PRESET_DISCOVERY_DEVICE_TYPE_INSTRUMENT);

    preset_discovery_file_type_t fileType = host->create_preset_file_type(
        "surge_preset", "Surge Preset", "fxp", surgePresetMetadataParser, nullptr);
    ...
    if (!SHGetKnownFolderPath(FOLDERID_ProgramData, 0, nullptr, &localAppData))
    {
        ...
        preset_discovery_location_t factoryPresets = host->create_preset_location(path);
        ...
        preset_discovery_location_t thirdPartyPresets = host->create_preset_location(path);
    ...
    if (!SHGetKnownFolderPath(FOLDERID_Documents, 0, nullptr, &documentsFolder))
    {
        ...
        preset_discovery_location_t userPresets = host->create_preset_location(path);
...
void preset_discovery_terminate() 
{
}